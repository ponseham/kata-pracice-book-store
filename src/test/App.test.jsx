...
import App from '../App'
import { TEST_BOOKS, TEST_HEADER, TEST_BOOK_PRICE, TEST_CURRENCY_LABEL, TEST_MIX_AND_SAVE_TEXT, TEST_DISCOUNT_INFO_TEXT, TEST_BASKET_SECTION_TITLE, TEST_BASKET_EMPTY_MESSAGE, TEST_ADD_BUTTON_LABEL, TEST_ADD_BOOK_TO_BASKET_AREA_LABEL, TESTING_TEST_ID_BASKET_ITEM, TEST_REMOVE_BUTTON_LABEL, TEST_REMOVE_BUTTON_AREA_LABEL, TEST_CLEAR_BUTTON_LABEL, TEST_CLEAR_BASKET_ARIA_LABEL, TEST_IN_BASKET_SUFFIX, TEST_ITEMS, TEST_ITEM, TEST_ADD_ONE_MORE_PREFIX, TEST_SUBTOTAL_LABEL, TEST_TOTAL_LABEL, TEST_ONE_BOOK_WITHOUT_DISCOUNT_AMOUNT, TEST_DISCOUNT_LABEL, TEST_TWO_BOOK_WITH_DISCOUNT_TOTAL, TEST_TWO_BOOK_WITH_DISCOUNT_SUBTOTAL, TEST_TWO_BOOK_WITH_DISCOUNT_DISCOUNT, TEST_THREE_BOOK_WITH_DISCOUNT_TOTAL, TEST_THREE_BOOK_WITH_DISCOUNT_SUBTOTAL, TEST_THREE_BOOK_WITH_DISCOUNT_DISCOUNT, TEST_FOUR_BOOK_WITH_DISCOUNT_TOTAL, TEST_FOUR_BOOK_WITH_DISCOUNT_SUBTOTAL, TEST_FOUR_BOOK_WITH_DISCOUNT_DISCOUNT, TEST_FIVE_BOOK_WITH_DISCOUNT_TOTAL, TEST_FIVE_BOOK_WITH_DISCOUNT_SUBTOTAL, TEST_FIVE_BOOK_WITH_DISCOUNT_DISCOUNT, TEST_LESS_SEVEN_BOOK_WITH_DISCOUNT_TOTAL, TEST_LESS_SEVEN_BOOK_WITH_DISCOUNT_SUBTOTAL, TEST_LESS_SEVEN_BOOK_WITH_DISCOUNT_DISCOUNT, TEST_EIGHT_OR_MORE_BOOKS_WITH_DISCOUNT_TOTAL, TEST_EIGHT_OR_MORE_BOOKS_WITH_DISCOUNT_SUBTOTAL, TEST_EIGHT_OR_MORE_BOOKS_WITH_DISCOUNT_DISCOUNT } from '../constants/testingConstants'
...
describe('Book Store', () => {
    ...
    test("Show price with discount when selecting three different books", async () => {
        ...
        expect(screen.getByText(TEST_DISCOUNT_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_THREE_BOOK_WITH_DISCOUNT_TOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_THREE_BOOK_WITH_DISCOUNT_SUBTOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_THREE_BOOK_WITH_DISCOUNT_DISCOUNT + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
    });
    test("Show price with discount when selecting four different books", async () => {
        renderWithStore()
        await addGivenBooksToBasket([0, 1, 2, 3])
        expect(screen.getByText(TEST_DISCOUNT_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_FOUR_BOOK_WITH_DISCOUNT_TOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_FOUR_BOOK_WITH_DISCOUNT_SUBTOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_FOUR_BOOK_WITH_DISCOUNT_DISCOUNT + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
    });
    test("Show price with discount when selecting five different books", async () => {
        renderWithStore()
        await addGivenBooksToBasket([0, 1, 2, 3, 4])
        expect(screen.getByText(TEST_DISCOUNT_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_FIVE_BOOK_WITH_DISCOUNT_TOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_FIVE_BOOK_WITH_DISCOUNT_SUBTOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_FIVE_BOOK_WITH_DISCOUNT_DISCOUNT + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
    });
    test("Show price with discount when selecting less or equal to seven books", async () => {
        renderWithStore()
        await addGivenBooksToBasket([0, 1, 2, 3, 4, 1, 2])
        expect(screen.getByText(TEST_DISCOUNT_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_LESS_SEVEN_BOOK_WITH_DISCOUNT_TOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_LESS_SEVEN_BOOK_WITH_DISCOUNT_SUBTOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_LESS_SEVEN_BOOK_WITH_DISCOUNT_DISCOUNT + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
    });
    test("Calculate discount for selecting sets of books total books more than seven books", async () => {
        ...
        expect(screen.getByText(TEST_DISCOUNT_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_EIGHT_OR_MORE_BOOKS_WITH_DISCOUNT_TOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_EIGHT_OR_MORE_BOOKS_WITH_DISCOUNT_SUBTOTAL + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
        expect(screen.getByText(TEST_EIGHT_OR_MORE_BOOKS_WITH_DISCOUNT_DISCOUNT + ' ' + TEST_CURRENCY_LABEL)).toBeInTheDocument()
    });
})
